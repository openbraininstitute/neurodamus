...
Both the `support` ones are bundled with the neurodamus install, so the user only needs to supply the `scientifc` ones.
...
Will compile the files within `some-mod-dir` along with the support MOD files since `--internal-mods` was specified.
...
EX::
    ...
    SPECIALS_PATH=/some/path/to/where/the/speciels/are
