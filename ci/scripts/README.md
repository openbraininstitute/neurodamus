...
Each of the scripts follows the same pattern, it is sourced and then one of the functions within it is executed.
...
Not every step is required to be executed, for instance `hdf5` might be installable by the system package manager so it does not need to be compiled.
