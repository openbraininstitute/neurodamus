...
def test_neuron_single(create_tmp_simulation_config_file):
    ...
    subprocess.run(command, check=True, capture_output=True)
    for t in [0, 10]:
        assert Path(f"output/1_py_Neuron_t{t:0.1f}.nrndat").exists()
...
def test_coreneuron(create_tmp_simulation_config_file):
    ...
    subprocess.run(command, check=True, capture_output=True)
    assert Path("output/3_cpu_init.corenrn").exists()
    assert Path("output/3_cpu_t0.000000.corenrn").exists()
    assert Path("output/3_cpu_t10.000000.corenrn").exists()
