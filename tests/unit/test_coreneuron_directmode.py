...
def test_coreneuron_filemode(create_tmp_simulation_config_file):
    ...
    assert not Path(CoreConfig.sim_config_file).exists()
    assert not Path(CoreConfig.report_config_file_save).exists()

    # check out.dat generated by CoreNEURON
    assert (Path(CoreConfig.output_root) / "out.dat").exists()
    spikes = np.loadtxt(
        Path(CoreConfig.output_root) / "out.dat", dtype=[("time", "double"), ("gid", "uint32")]
    )
    times = spikes["time"]
    gids = spikes["gid"]
...
def test_coreneuron_directmode(create_tmp_simulation_config_file):
    ...
    assert not (Path(CoreConfig.output_root) / "out.dat").exists()
...
def test_coreneuron_disable_report(create_tmp_simulation_config_file):
    ...
    assert Path(CoreConfig.sim_config_file).exists()
    assert not Path(CoreConfig.report_config_file_save).exists()
    assert not Path(SimConfig.populations_offset_save_path()).exists()
