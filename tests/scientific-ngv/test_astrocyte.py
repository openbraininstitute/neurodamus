...
def test_base():
    ...
    assert all(isinstance(g, glut_type) for g in a._glut_list), "All elements in _glut_list must be GlutReceive"
    assert len(full_glut_list) == len(a._glut_list) + 1, "glut_list should be exactly one longer than _glut_list"
    assert all(isinstance(g, glut_type) for g in full_glut_list[:-1]), "All but last in glut_list must be GlutReceive"
    assert isinstance(full_glut_list[-1], soma_type), "Last element in glut_list must be GlutReceiveSoma"